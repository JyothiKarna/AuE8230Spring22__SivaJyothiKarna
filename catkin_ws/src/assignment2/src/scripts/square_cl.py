...
def square_cl(square_x,square_y, speed):
     print('square_cl function has been initiated')
     pub = rospy.Publisher('/turtle1/cmd_vel', Twist, queue_size =10)
     vel_msg = Twist()
     ...
     while not rospy.is_shutdown():
           #t0 = float(rospy.Time.now().to_sec())
           while (sqrt(pow((square_x - pos_x), 2) + pow((square_y - pos_y), 2))) >= distance_tolerance:

                  distance = abs(math.sqrt(((square_x-pos_x) ** 2) + ((square_y-pos_y) ** 2)))
                  vel_msg.linear.x = speed*0.5 * distance
                  vel_msg.angular.z = 3.0 * (atan2(square_y - pos_y, square_x - pos_x) - pos_yaw) #(math.atan2(square_y-pos_y, square_x-pos_x)-pos_yaw)
